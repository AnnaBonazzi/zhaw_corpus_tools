...
Script to make a plot of corpus words frequency in each corpus class or language, depending on the format of the input data.

Every keyword has its frequency dic with frequency per class. The classes could be replaced by language data.

For more / less words: add/remove a new keyword, a new dic, and a new "make_bar"-function at line 67

For more / less bar groups: 
'''
...
number_of_bar_groups = 4 # For classes (4) or for languages
...
for i in range(1, number_of_bar_groups):
	...
	xlist.append(n)
#xlist = [2, 5, 8, 11]
def make_bar(dic, dist, bar_color):
	ax.bar([x + dist for x in xlist], dic.values(),width=0.3, align='center', color=bar_color) 
	
make_bar(dic1, -(dist*4), bar_colors[keyw1])
make_bar(dic2, -(dist*3), bar_colors[keyw2])
make_bar(dic3, -(dist*2), bar_colors[keyw3])
make_bar(dic4, -(dist), bar_colors[keyw4])
make_bar(dic5, 0, bar_colors[keyw5])
make_bar(dic6, dist, bar_colors[keyw6])
make_bar(dic7, dist*2, bar_colors[keyw7])
make_bar(dic8, dist*3, bar_colors[keyw8])
...
# Set legend
leg = plt.legend(loc='best', frameon=False, labels = [keyw1, keyw2, keyw3, keyw4, keyw5, keyw6, keyw7, keyw8]) # edgecolor = 'b', facecolor = 'r',
